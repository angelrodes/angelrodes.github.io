...
Exposure_model = @(Prodution_rates,Attenuation_lengths,decay_constant,t)...
   sum( (Prodution_rates./decay_constant).*(1-exp(-decay_constant.*t)) ,1);
Exposure_model10 = @(t) Exposure_model(P10,L,l10,t);
Exposure_model26 = @(t) Exposure_model(P26,L,l26,t);

many_times=[0, logspace(0,8,1000-1)]; % years

x_exposure=Exposure_model10(many_times);
y_exposure=Exposure_model26(many_times)./Exposure_model10(many_times);
...
time_array=[0 0.1 0.5 1 1.1 1.2 1.3 1.4 1.5 2 3 4 6 8 10 15 20]*1E6; % years (Ma multiplied by 1E6)
% WE ADDED A FEW EXTRA LINES HERE (1.1 1.2 1.3 1.4) TO UNDERSTAND THE BURIAL AGE OF THE SAMPLE
...
xlabel('Be-10 *')
ylabel('Al-26/Be-10 *')
title('BANANA (*)all scaled to production rate ratios')

% plot constant-exposure line
plot(x_exposure/sum(P10),y_exposure/sum(P26)*sum(P10),'-k')

% plot erosion rates
for er=erosion_array
  x=Model10(er,many_times)/sum(P10);
  y=Model_ratio(er,many_times)/sum(P26)*sum(P10);
...
% plot times
for t=time_array
  x=Model10(many_erosions,t)/sum(P10);
  y=Model_ratio(many_erosions,t)/sum(P26)*sum(P10);
...
% Set nice limits
xlim([ 100 Model10(0,0)*2]/sum(P10)) % between 100 atoms/g and 20% more than saturation
...
plot(C10/sum(P10),C26/C10/sum(P26)*sum(P10),'.k') % center of the sample (k is for black)
plot([C10-dC10 , C10+dC10]/sum(P10) , [C26/C10 , C26/C10]/sum(P26)*sum(P10),'-k') % horizontal uncertainty
plot([C10 , C10]/sum(P10) , [(C26-dC26)/C10 , (C26+dC26)/C10]/sum(P26)*sum(P10),'-k') % vertical uncertainty
...
plot(x/sum(P10),y/sum(P26)*sum(P10),'-g') % green ellipse
text(x(1)/sum(P10),y(1)/sum(P26)*sum(P10),sample_name,'Color','g') % green text
% maths behind this:
...
% plot guess results
plot(Model10(guess_er,guess_t)/sum(P10),Model_ratio(guess_er,guess_t)/sum(P26)*sum(P10),'*m') % magenta star
...
% To get those results, you will need to put the "solver" above in a loop for all the objectives we want to fit. (below)
...
%    - Represent everything scaled to the production rate ratios sum(P10) and sum(P26). This is useful if you want to represent several samples (with different basin production rates) in the same banana. (DONE)
...
%% UNCERTINTIES
% Run the solver for the points on the ellipse
...
% our objective is guetting the er and t values that yield models that correposnd to our concentrations
% therefore, our objectives are the sample conventrations:
Obj_10=C10+dC10*sin(theta);
Obj_26=C26+dC26*cos(theta);
% and their uncertainties
dObj_10=dC10;
dObj_26=dC26;
...
time_solutions(counter)=guess_t;
erosion_solutions(counter)=guess_er;

end

%% Plot the min-max solutions on the ellipse
sel= (time_solutions==max(time_solutions)); % select the max age
plot(Model10(erosion_solutions(sel),time_solutions(sel))/sum(P10),Model_ratio(erosion_solutions(sel),time_solutions(sel))/sum(P26)*sum(P10),'*r') % magenta star

sel= (time_solutions==min(time_solutions)); % select the min age
plot(Model10(erosion_solutions(sel),time_solutions(sel))/sum(P10),Model_ratio(erosion_solutions(sel),time_solutions(sel))/sum(P26)*sum(P10),'*r') % magenta star

sel= (erosion_solutions==max(erosion_solutions)); % select the max erosion
plot(Model10(erosion_solutions(sel),time_solutions(sel))/sum(P10),Model_ratio(erosion_solutions(sel),time_solutions(sel))/sum(P26)*sum(P10),'*b') % magenta star

sel= (erosion_solutions==min(erosion_solutions)); % select the min erosion
plot(Model10(erosion_solutions(sel),time_solutions(sel))/sum(P10),Model_ratio(erosion_solutions(sel),time_solutions(sel))/sum(P26)*sum(P10),'*b') % magenta star

disp('One-sigma ranges:')
disp(['    Burial ages = ' num2str(min(time_solutions)/1e6) ' - '  num2str(max(time_solutions)/1e6) ' Ma'])
disp(['    Erosion ratess = ' num2str(min(erosion_solutions)*10000) ' - '  num2str(max(erosion_solutions)*10000) 'm/Ma'])